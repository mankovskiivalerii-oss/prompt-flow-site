  ...
  return (
    <div className="page">
      ...
      <main>
        ...
        <section className="section alt" id="request"><div className="container"><h2>Форма заявки</h2><form className="request-form" onSubmit={handleSubmit}><label>Имя<input name="name" type="text" placeholder="Ваше имя" required /></label><label>Контакт<input name="contact" type="text" placeholder="Телефон или Telegram" required /></label><label>Описание задачи<textarea name="message" placeholder="Кратко опишите задачу" rows={4} required /></label><button className="btn" type="submit" disabled={isSubmitting}>{isSubmitting ? 'Отправка…' : 'Отправить заявку'}</button>{formStatus === 'success' && <p className="form-message success">Спасибо! Заявка отправлена. Мы свяжемся с вами в ближайшее время.</p>}{formStatus === 'error' && <p className="form-message error">Не удалось подтвердить отправку. Попробуйте ещё раз или напишите напрямую на email: mankovskiivalerii@gmail.com</p>}</form><p className="form-fallback-contact">Если форма не отправляется, напишите напрямую: mankovskiivalerii@gmail.com</p></div></section>
      </main>
